...
let time = new Date();
!(async () => {
  if ($request.url.includes("nt2.ele.me")) {
    var Cookie = $request.headers.Cookie; // 从请求头中获取cookie
    let token = await getToken(ql);
    var result = await SetEnvs(ql, token, $.name, Cookie);
    $notify("饿了么ck", $.name, result);
  }
})()
.catch((err) => $.logErr(err))
.finally(() => $.done());
async function getToken(config) {
  return new Promise((resolve, reject) => { // 添加 reject 处理错误
    var request = {
      url: config.url + "/open/auth/token?client_id=" + config.id + "&client_secret=" + config.secret,
    };
    $.get(request, (error, response, data) => { // 移除 async，因为这里不需要
      if (error) {
        ...
        try {
          ...
          if (result.code === 200) { // 确保响应的结果是成功的
            ...
            reject(new Error("API 返回非200状态码")); // 处理非200状态码的情况
          }
        } catch (e) {
...
async function SetEnvs(config, token, name, cookie) {
  return new Promise((resolve) => {
    var request = {
      ...
      body: `[{"value":"${cookie}","name":"${name}","remarks":"QX_${time.toLocaleString()}"}]`,
    };
